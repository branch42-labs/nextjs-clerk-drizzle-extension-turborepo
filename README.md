...
1. Add `CLERK_SECRET_KEY` and app publishable keys in `.env` (see `.env.example`).
2. Add `CLERK_WEBHOOK_SIGNING_SECRET` if you want Clerk user lifecycle events synced to your DB.
3. In Clerk Dashboard, configure OAuth providers and webhook endpoint:
...
### 4. Configuring Clerk for Expo

For Expo OAuth sign-in, ensure your Clerk app has the same redirect scheme as your Expo app (`expo://` by default in this template).

In development, if Expo talks to your local Next.js backend, point Expo's API base URL to that machine/port.
...
This template exposes a Clerk webhook endpoint at `POST /api/webhooks/clerk` (Next.js app) for syncing user lifecycle events into your database.
