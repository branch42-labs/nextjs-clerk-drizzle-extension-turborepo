...
function MobileAuth() {
  ...
  const handleAuthPress = async () => {
    ...
    try {
      ...
      console.error("Failed to start Clerk OAuth flow", error);
    }
  };
  ...
  return (
    <>
      <Text className="text-foreground pb-2 text-center text-xl font-semibold">
        {isSignedIn
          ? `Hello, ${user?.fullName ?? user?.primaryEmailAddress?.emailAddress ?? "User"}`
          : "Not logged in"}
      </Text>
      <Pressable
        ...
        className="bg-primary flex items-center rounded-sm p-2"
      >
        <Text>{isSignedIn ? "Sign Out" : "Sign In With Clerk"}</Text>
      </Pressable>
    </>
  );
}
